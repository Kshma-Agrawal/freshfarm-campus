...
import {
  CaretLeft, CaretRight, MapPin, Check,
  Eye, EyeSlash, Funnel, SignOut, Trash
...
export function FarmerProfilePage() {
  ...
  const menuItems = [
    { icon: '🚜', label: 'Farm Details', path: '/farmer/farm-details' },
    { icon: '🔖', label: 'Bookmarks', path: '/farmer/bookmarks' },
    { icon: '🏦', label: 'Payment Details', path: '/farmer/payment-details' },
    { icon: '🛡️', label: 'Verification Status', path: '/farmer/verification-pending' },
    { icon: '💰', label: 'Earnings', path: '/farmer/earnings' },
    { icon: '🛒', label: 'My Purchases', path: '/farmer/my-purchases' },
  ]
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <div className="top-nav">
        <div className="logo-area">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <MapPin size={13} weight="fill" color="var(--green-main)" />
            <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--green-main)' }}>Gurgaon , Haryana</span>
            <CaretRight size={13} color="var(--green-main)" />
          </div>
        </div>
      ...
      <div className="screen">
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 20px 24px', background: 'white' }}>
          <div style={{ width: 88, height: 88, borderRadius: '50%', background: 'var(--green-light)', position: 'relative', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 52 }}>
            🧑‍🌾
            <div style={{ position: 'absolute', bottom: 4, right: 4, width: 24, height: 24, borderRadius: '50%', background: 'var(--green-main)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Check size={13} color="white" weight="bold" />
            </div>
          </div>
          <div style={{ fontSize: 20, fontWeight: 700 }}>Harpal Singh</div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--green-light)', color: 'var(--green-main)', padding: '5px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, marginTop: 8 }}>
        ...
        <div style={{ marginTop: 12 }}>
          {menuItems.map(({ icon, label, path }) => (
            <div key={label} className="profile-menu-item" onClick={() => navigate(path)}>
              <div className="profile-menu-icon" style={{ fontSize: 20 }}>{icon}</div>
      ...
      <div style={{ padding: '16px 20px 100px' }}>
        {[
          ...
          <div key={f.label} style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-light)', marginBottom: 6, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{f.label}</label>
            <div style={{ padding: '14px 16px', background: 'var(--bg)', borderRadius: 'var(--radius)', fontSize: 15, fontWeight: 500, color: 'var(--text-dark)' }}>
              {f.prefix}{f.value}
            </div>
          </div>
        ))}
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
          {[{ label: 'Farm Size (Acres)', value: '5' }, { label: 'Experience (Years)', value: '10' }].map(f => (
            <div key={f.label} style={{ flex: 1 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-light)', marginBottom: 6, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{f.label}</label>
              <div style={{ padding: '14px 16px', background: 'var(--bg)', borderRadius: 'var(--radius)', fontSize: 15, fontWeight: 500 }}>{f.value}</div>
            </div>
          ))}
        </div>

        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-light)', marginBottom: 10, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Categories</label>
        ...
        <div style={{ marginBottom: 28 }}>
          <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-light)', marginBottom: 10, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Farm Photos</label>
          <div style={{ display: 'flex', gap: 10 }}>
            {photos.map((src, i) => (
              <div key={i} style={{ width: 100, height: 80, borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
  ...
  return (
    <div style={{ background: 'var(--white)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '16px 20px 100px' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-mid)', marginBottom: 10, display: 'block' }}>Categories</label>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {allCats.map(c => (
        ...
        <div style={{ marginBottom: 28 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-mid)', marginBottom: 10, display: 'block' }}>Farm Photos</label>
          <div style={{ display: 'flex', gap: 10 }}>
      ...
      <div style={{ padding: '16px 20px 100px' }}>
        {fields.map(f => (
          <div key={f.label} style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-light)', marginBottom: 6, display: 'block', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{f.label}</label>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: 'var(--bg)', borderRadius: 'var(--radius)' }}>
              <span style={{ fontSize: 15, fontWeight: 500 }}>{f.value}</span>
              {f.toggleable && (
                <button onClick={() => setShowAcct(!showAcct)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-light)' }}>
                  {showAcct ? <EyeSlash size={18} /> : <Eye size={18} />}
                </button>
              )}
              {f.copyable && (
                <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-light)', fontSize: 16 }}>📋</button>
              )}
              {f.badge && (
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green-main)', background: 'var(--green-light)', padding: '3px 10px', borderRadius: 10 }}>{f.badge}</span>
              )}
            </div>
          </div>
        ))}
  ...
  return (
    <div style={{ background: 'var(--white)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '16px 20px 100px' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-mid)', marginBottom: 6, display: 'block' }}>Account Number</label>
          <div className="input-wrap" style={{ marginBottom: 0 }}>
            <input className="input-field no-icon" defaultValue={showAcct ? '1234567890001234' : '···· ···· ···· 1234'} style={{ paddingRight: 44 }} />
            <button onClick={() => setShowAcct(!showAcct)} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-light)' }}>
              {showAcct ? <EyeSlash size={18} /> : <Eye size={18} />}
            </button>
          </div>
        </div>
...
export function FarmerSettingsPage() {
  ...
  const Toggle = ({ on, onToggle }) => (
    <button onClick={onToggle} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
      <div style={{ width: 48, height: 26, borderRadius: 13, background: on ? 'var(--green-main)' : 'var(--border)', position: 'relative', transition: 'background 0.2s' }}>
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      {/* Avatar */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 20px 24px', background: 'white', marginBottom: 16 }}>
        <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'var(--green-light)', position: 'relative', marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 46 }}>
          🧑‍🌾
          <div style={{ position: 'absolute', bottom: 2, right: 2, width: 22, height: 22, borderRadius: '50%', background: 'var(--green-main)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Check size={12} color="white" weight="bold" />
          </div>
        </div>
        <div style={{ fontWeight: 700, fontSize: 18 }}>Harpal Singh</div>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--green-light)', color: 'var(--green-main)', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, marginTop: 6 }}>
          <Check size={12} weight="bold" /> Verified farmer
        </div>
      </div>

      {/* General */}
      <div style={{ background: 'white', marginBottom: 8 }}>
        <div style={{ padding: '10px 20px 4px', fontSize: 11, fontWeight: 700, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>GENERAL</div>
        <div className="profile-menu-item" style={{ justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-light)', fontSize: 14 }}>
      ...
      {/* Notifications */}
      <div style={{ background: 'white', marginBottom: 8 }}>
        <div style={{ padding: '10px 20px 4px', fontSize: 11, fontWeight: 700, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>NOTIFICATIONS</div>
        {[
          ...
          <div key={n.key} className="profile-menu-item" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 500, fontSize: 15 }}>{n.label}</div>
              <div style={{ fontSize: 12, color: 'var(--text-light)', marginTop: 2 }}>{n.desc}</div>
            </div>
      ...
      <div style={{ background: 'white' }}>
        <div style={{ padding: '10px 20px 4px', fontSize: 11, fontWeight: 700, color: 'var(--red)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>ACCOUNT ACTIONS</div>
        <div className="profile-menu-item" onClick={() => navigate('/farmer/login')} style={{ gap: 14 }}>
          <div className="profile-menu-icon" style={{ background: '#ffebee' }}><SignOut size={18} color="var(--red)" /></div>
          <span style={{ fontWeight: 500, color: 'var(--text-dark)' }}>Log out</span>
        </div>
        <div className="profile-menu-item" style={{ gap: 14 }}>
          <div className="profile-menu-icon" style={{ background: '#ffebee' }}><Trash size={18} color="var(--red)" /></div>
          <span style={{ fontWeight: 500, color: 'var(--red)' }}>Delete Account</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '8px 16px 100px' }}>
        {/* Total earnings card */}
        <div style={{ background: 'var(--green-main)', borderRadius: 'var(--radius-lg)', padding: '24px 20px', marginBottom: 16, color: 'white' }}>
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 16, marginBottom: 16, boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            {[{ icon: '🛒', label: 'PRODUCT SALES', value: '₹10,000', sub: 'This month' },
              { icon: '🌿', label: 'MATERIAL SALES', value: '₹13,200', sub: 'This month' }].map(c => (
              <div key={c.label} style={{ flex: 1, background: 'var(--bg)', borderRadius: 'var(--radius)', padding: '14px 12px', textAlign: 'center' }}>
                <div style={{ fontSize: 22, marginBottom: 4 }}>{c.icon}</div>
                <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{c.label}</div>
                <div style={{ fontSize: 18, fontWeight: 800, margin: '4px 0 2px' }}>{c.value}</div>
                <div style={{ fontSize: 11, color: 'var(--text-light)' }}>{c.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 80, marginTop: 20, paddingTop: 10 }}>
            {days.map((d, i) => (
              <div key={d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ width: '100%', background: i === 3 ? 'var(--green-main)' : 'var(--green-light)', borderRadius: '4px 4px 0 0', height: `${heights[i]}%`, minHeight: 4 }} />
                <span style={{ fontSize: 9, color: i === 3 ? 'var(--green-main)' : 'var(--text-light)', fontWeight: i === 3 ? 700 : 400 }}>{d}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', padding: 16, boxShadow: 'var(--shadow-sm)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button onClick={() => navigate('/farmer/transactions')} style={{ background: 'none', border: 'none', color: 'var(--green-main)', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>See All</button>
          </div>
          {transactions.map((t, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: i < transactions.length - 1 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 38, height: 38, borderRadius: '50%', background: t.positive ? 'var(--green-light)' : '#ffebee', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
                  {t.positive ? '➕' : '🏦'}
                </div>
                <div>
                  <div style={{ fontSize: 12, color: 'var(--text-light)', marginBottom: 2 }}>{t.date}</div>
                  <div style={{ fontWeight: 600, fontSize: 13 }}>{t.label}</div>
                </div>
              </div>
              <div style={{ fontWeight: 700, fontSize: 15, color: t.positive ? 'var(--green-main)' : 'var(--red)' }}>{t.amount}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ paddingBottom: 100 }}>
        {transactions.map((t, i) => (
          <div key={i} onClick={() => navigate('/farmer/transaction/1')}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 42, height: 42, borderRadius: '50%', background: t.positive ? 'var(--green-light)' : '#ffebee', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                {t.positive ? '➕' : '🏦'}
              </div>
              <div>
                <div style={{ fontSize: 13, color: 'var(--text-light)', marginBottom: 2 }}>{t.date}</div>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{t.label}</div>
              </div>
            </div>
            <div style={{ fontWeight: 700, fontSize: 16, color: t.positive ? 'var(--green-main)' : 'var(--red)' }}>{t.amount}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '28px 20px 100px', textAlign: 'center' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--green-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <Check size={32} color="var(--green-main)" weight="bold" />
          </div>
          <div style={{ fontSize: 36, fontWeight: 800, color: 'var(--green-main)' }}>+₹2,400</div>
          <span style={{ fontSize: 12, fontWeight: 700, background: 'var(--green-light)', color: 'var(--green-main)', padding: '4px 14px', borderRadius: 20, marginTop: 8, display: 'inline-block' }}>SETTLED</span>
          <div style={{ fontSize: 13, color: 'var(--text-light)', marginTop: 8 }}>Transaction ID: TXN_FF_99210</div>
          <div style={{ fontSize: 12, color: 'var(--text-light)', marginTop: 3 }}>Oct 24, 2023, 10:45 AM</div>
        </div>

        {/* Details */}
        <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius-lg)', padding: '20px', textAlign: 'left', marginBottom: 16 }}>
          {[
            ...
            <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14, paddingBottom: 14, borderBottom: '1px solid var(--border)' }}>
              <span style={{ fontSize: 14, color: 'var(--text-light)' }}>{r.label}</span>
        ...
        {/* Breakdown */}
        <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius-lg)', padding: '20px', textAlign: 'left', marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 14 }}>BREAKDOWN</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <div>
              <div style={{ fontWeight: 600, fontSize: 14 }}>Organic Tomatoes (20kg)</div>
              <div style={{ fontSize: 12, color: 'var(--text-light)' }}>Unit Price: ₹120/kg</div>
            </div>
        ...
        {/* Bank */}
        <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius-lg)', padding: '16px 20px', textAlign: 'left', marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontSize: 24 }}>🏦</div>
            <div>
              <div style={{ fontSize: 12, color: 'var(--text-light)', marginBottom: 2 }}>Payment Mode</div>
              <div style={{ fontWeight: 700, fontSize: 14 }}>HDFC Bank</div>
              <div style={{ fontSize: 12, color: 'var(--text-light)' }}>Settled to Primary Account</div>
            </div>
          </div>
        </div>
