...
// ── TOP NAV ───────────────────────────────────────────────────────
export function TopNav({ showLocation = true, location = 'Gurgaon, Haryana', farmerMode = false }) {
  const navigate = useNavigate()

  const notifPath = farmerMode ? '/farmer/notifications' : '/notifications'
  return (
    <div className="top-nav">
      ...
      <button className="icon-btn" onClick={() => navigate(notifPath)}>
        <Bell size={22} />
      </button>
    </div>
  )
...
export function BottomNav() {
  ...
  const items = [
    { key: '/home',    IconInactive: House,         IconActive: House,         label: 'Home',    activeWeight: 'fill' },
    { key: '/search',  IconInactive: MagnifyingGlass, IconActive: MagnifyingGlass, label: 'Search', activeWeight: 'fill' },
    { key: '/cart',    IconInactive: ShoppingCart,  IconActive: ShoppingCart,  label: 'Cart',    activeWeight: 'fill' },
    { key: '/profile', IconInactive: User,           IconActive: User,          label: 'Profile', activeWeight: 'fill' },
  ]
  const active = items.find(i => i.key === location.pathname)?.key || '/home'

  return (
    <nav className="bottom-nav">
      {items.map(({ key, IconInactive, IconActive, label, activeWeight }) => {
        const isActive = active === key
        const IconComp = isActive ? IconActive : IconInactive
        return (
          <button key={key} className={`nav-item ${isActive ? 'active' : ''}`} onClick={() => navigate(key)}>
            <span className="nav-icon">
              <IconComp size={26} weight={isActive ? activeWeight : 'regular'} />
              {key === '/cart' && cartCount > 0 && <span className="badge">{cartCount}</span>}
            </span>
            <span>{label}</span>
