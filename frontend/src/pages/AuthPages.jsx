...
import { GoogleIcon, CaretLeft, Lock, Phone, EnvelopeSimple, Check, User } from '../components/UI'
  ...
  return (
    <div className="splash-screen screen no-nav">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, width: '100%' }}>
        <p style={{ fontSize: 15, color: 'var(--text-subtle)', lineHeight: 1.6, textAlign: 'center', maxWidth: 260, fontWeight: 500 }}>
          Fresh produce delivered straight from local farms to your door
        </p>
        <button className="btn btn-primary mt-24"onClick={() => navigate('/select-role')}>Get Started</button>
      </div>
    </div>
  )
}
...
export function SelectRolePage() {
  ...
  return (
    <div className="screen no-nav" style={{ padding: '48px 24px 32px', background: 'var(--white)' }}>
      ...
      <button className="btn btn-primary mt-24" onClick={() => navigate(role === 'farmer' ? '/farmer/login' : '/login')}>Continue</button>
    </div>
  )
}
