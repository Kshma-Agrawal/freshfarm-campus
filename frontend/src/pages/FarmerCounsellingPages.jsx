...
import {
  CaretLeft, CaretRight, CaretDown, MagnifyingGlass, Bookmark, ShareNetwork, TrendUp, ArrowUpRight, MapPin, Bell, PencilSimple
} from '@phosphor-icons/react'
import { FarmerBottomNav } from './FarmerOnboardingPages'
import farmerFieldImg from '../assets/farmer-sunrise.jpg'
import farmerRaniImg from '../assets/farmer-rani.jpg'
import kaleImg from '../assets/product-kale.jpg'
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div className="screen">
        <div style={{ padding: '16px 20px' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--text-subtle)', marginBottom: 20 }}>Expert guidance for your agricultural journey.</div>
        </div>
        {[
          { img: farmerFieldImg, title: 'Demand Insights', desc: 'Analyze current market trends and predict upcoming crop demand for better profits.', cta: 'View Insights', path: '/farmer/demand-insights' },
          { img: farmerRaniImg, title: 'Crop Planning', desc: 'Personalized calendars for your soil type, season, and regional climate conditions.', cta: 'Start Planning', path: '/farmer/crop-planning' },
        ].map((card, i) => (
          <div key={i} style={{ margin: '0 16px 16px', background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div style={{ height: 160, overflow: 'hidden' }}>
              <img src={card.img} alt={card.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <div style={{ padding: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 6 }}>{card.title}</div>
              <div style={{ fontSize: 13, color: 'var(--text-subtle)', lineHeight: 1.6, marginBottom: 14 }}>{card.desc}</div>
...
export function DemandInsightsPage() {
  ...
  const prices = [
    { name: 'Organic Kale', img: kaleImg, price: '40/kg', change: '+4%', up: true },
    { name: 'Avocados', img: carrotImg, price: '50/kg', change: '-2%', up: false },
    { name: 'Tomatoes', img: tomatoImg, price: '35/kg', change: '+8%', up: true },
    { name: 'Baby Spinach', img: spinachImg, price: '55/kg', change: '+1%', up: true },
  ]
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '8px 16px 100px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
          <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'white', border: '1.5px solid var(--border)', borderRadius: '8px', fontFamily: 'var(--font)', fontSize: 13, cursor: 'pointer' }}>
            <MapPin size={13} weight="fill" /> Gurgaon, Ha… <CaretDown size={14} />
          </button>
          <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'white', border: '1.5px solid var(--border)', borderRadius: '8px', fontFamily: 'var(--font)', fontSize: 13, cursor: 'pointer' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '16px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>MARKET OVERVIEW</div>
              <div style={{ fontSize: 17, fontWeight: 700, marginTop: 4 }}>Trending Crops</div>
            </div>
            <span style={{ background: '#e8f5e9', color: 'var(--primary)', fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 4 }}><TrendUp size={13} /> +12.4%</span>
          </div>
          {/* Bar chart */}
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', height: 100, marginBottom: 8 }}>
            {crops.map((c, i) => (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ width: '100%', background: i === 3 ? 'var(--primary)' : 'var(--primary-light)', borderRadius: '6px 6px 0 0', height: `${c.h}%`, minHeight: 8, transition: 'height 0.3s' }} />
                <span style={{ fontSize: 9, color: 'var(--text-subtle)', fontWeight: 600 }}>{c.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: 16, marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button style={{ background: 'none', border: 'none', color: 'var(--primary)', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>View Details</button>
          </div>
          {prices.map((p, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 0', borderBottom: i < prices.length - 1 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, overflow: 'hidden', flexShrink: 0 }}><img src={p.img} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{p.name}</div>
                <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Retail Avg.</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 700, fontSize: 14 }}>{p.price}</div>
                <div style={{ fontSize: 12, fontWeight: 600, color: p.up ? 'var(--primary)' : 'var(--error)' }}>{p.change}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button style={{ background: 'none', border: 'none', color: 'var(--primary)', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>See All</button>
          </div>
          {[
            { name: 'Tomatoes', note: 'Short supply in Sacramento', img: '', badge: 'Critical', badgeColor: 'var(--error)' },
            ...
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 0', borderBottom: i < 1 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, overflow: 'hidden', flexShrink: 0 }}><img src={item.img} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{item.name}</div>
                <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>{item.note}</div>
              </div>
...
export function CropPlanningPage() {
  ...
  const plans = [
    { name: 'Spring Wheat 2024', detail: 'North Block - 42 Acres • Organic', edited: '2H AGO', status: 'OPTIMIZED', color: 'var(--primary)' },
    { name: 'Summer Barley 2024', detail: 'East Field - 30 Acres • Conventional', edited: '1H AGO', status: 'PLANNED', color: 'var(--primary)' },
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '4px 16px 100px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 13, color: 'var(--text-subtle)' }}>{plans.length} Total</span>
        </div>
        {plans.map((p, i) => (
          <div key={i} style={{ background: 'white', borderRadius: '12px', padding: '14px 16px', marginBottom: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 3 }}>{p.name}</div>
              <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginBottom: 6 }}>{p.detail}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>🕐 EDITED {p.edited}</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: p.color }}>● {p.status}</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={{ background: 'none', border: 'none', cursor: 'pointer' }} onClick={() => navigate('/farmer/create-plan')}><PencilSimple size={16} /></button>
  ...
  return (
    <div style={{ background: 'var(--white)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '0 0 100px' }}>
        {/* Hero */}
        <div style={{ margin: '0 20px 20px', borderRadius: '16px', overflow: 'hidden', height: 180, position: 'relative' }}>
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-subtle)', marginBottom: 10 }}>Select a Crop</div>
          <select value={crop} onChange={e => setCrop(e.target.value)}
            style={{ width: '100%', padding: '14px', border: '1.5px solid var(--border)', borderRadius: '12px', fontFamily: 'var(--font)', fontSize: 14, background: 'white', outline: 'none', marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 28 }}>
            {stats.map((s, i) => (
              <div key={i} style={{ background: 'var(--bg)', borderRadius: '12px', padding: '14px', textAlign: 'center' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>{s.label}</div>
...
export function BestPracticesPage() {
  ...
  const articles = [
    { title: 'Organic farming', desc: 'Learn the fundamentals of pesticide-free crop cultivation and soil health management for better yields.', img: farmerFieldImg, saved: false },
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '8px 16px 100px' }}>
        ...
        {articles.map((a, i) => (
          <div key={i} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div style={{ height: 170, position: 'relative', overflow: 'hidden' }}>
              ...
              <button onClick={() => setSavedStates(prev => prev.map((s, j) => j === i ? !s : s))}
                style={{ position: 'absolute', top: 12, right: 12, width: 36, height: 36, borderRadius: '50%', background: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <Bookmark size={18} weight={savedStates[i] ? 'fill' : 'regular'} color={savedStates[i] ? 'var(--primary)' : 'var(--text-dark)'} />
              </button>
            </div>
            <div style={{ padding: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 6 }}>{a.title}</div>
              <div style={{ fontSize: 13, color: 'var(--text-subtle)', lineHeight: 1.6, marginBottom: 14 }}>{a.desc}</div>
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          {['TECHNOLOGY', 'SUSTAINABILITY'].map(tag => (
            <span key={tag} style={{ fontSize: 11, fontWeight: 700, color: 'var(--primary)', background: 'var(--primary-light)', padding: '4px 10px', borderRadius: 6 }}>{tag}</span>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Published Oct 24, 2023 • 8 min read</div>
          </div>
        </div>

        <div style={{ fontSize: 15, color: 'var(--text-subtle)', lineHeight: 1.8 }}>
          <span style={{ fontSize: 48, fontWeight: 800, lineHeight: 0.8, float: 'left', marginRight: 8, color: 'var(--primary-dark)', fontFamily: 'Georgia, serif' }}>U</span>
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '8px 16px 100px' }}>
        ...
        {schemes.map((s, i) => (
          <div key={i} style={{ background: 'white', borderRadius: '16px', padding: 16, marginBottom: 14, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
              <div style={{ fontSize: 16, fontWeight: 700, flex: 1, paddingRight: 8 }}>{s.title}</div>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--primary)', background: 'var(--primary-light)', padding: '3px 10px', borderRadius: 10, flexShrink: 0 }}>ACTIVE</span>
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginBottom: 8 }}>Eligibility: {s.eligibility}</div>
            <div style={{ fontSize: 13, color: 'var(--text-subtle)', lineHeight: 1.6, marginBottom: 14 }}>{s.desc}</div>
...
export function BookmarksPage() {
  const navigate = useNavigate()
  const articles = [
    { title: 'Organic farming', desc: 'Learn the fundamentals of pesticide-free crop cultivation and soil health management for better yields.', img: farmerFieldImg },
        ...
        {articles.map((a, i) => (
          <div key={i} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div style={{ position: 'relative', height: 180, overflow: 'hidden' }}>
              <img src={a.img} alt={a.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              <button style={{ position: 'absolute', top: 12, right: 12, width: 36, height: 36, borderRadius: '50%', background: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <Bookmark size={18} weight="fill" color="var(--primary)" />
              </button>
            </div>
            <div style={{ padding: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 6 }}>{a.title}</div>
