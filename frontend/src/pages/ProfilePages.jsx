...
const farmerHarpalImg = 'https://images.unsplash.com/photo-1595475038784-bbe439ff41e6?w=400&q=80'
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {order.status === 'arrived' && (
                <>
                  ...
                  <button style={{ padding: '8px 14px', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'var(--primary)', color: 'white', fontFamily: 'var(--font)' }}
                    onClick={e => { e.stopPropagation() }}>Reorder</button>
                </>
              )}
              {order.status === 'ongoing' && (
                <button style={{ padding: '8px 16px', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'var(--primary)', color: 'white', fontFamily: 'var(--font)' }}
                  onClick={e => { e.stopPropagation(); navigate('/order-tracking') }}>Track Your Order</button>
              )}
...
export function OrderDetailPage() {
  ...
  return (
    <div className="screen" style={{ background: 'white' }}>
      ...
      <div style={{ background: 'var(--primary-pale)', borderRadius: 12, margin: '14px 20px 0', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 32, height: 32, borderRadius: '50%', background: isArrived ? 'var(--primary)' : '#fff8e1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {isArrived ? <Check size={16} color="white" weight="bold" /> : <span style={{ fontSize: 14 }}>🔄</span>}
        </div>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700 }}>Order #{order.id}</div>
          <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 2, fontWeight: 500 }}>₹{order.amount} • {order.date}</div>
          <div style={{ fontSize: 12, fontWeight: 600, color: isArrived ? 'var(--primary)' : '#f57f17', marginTop: 2 }}>{isArrived ? 'Arrived' : 'Ongoing'}</div>
        </div>
      </div>
      ...
      <div style={{ margin: '16px 20px', background: 'var(--bg)', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="contact-btn" style={{ padding: '10px 14px', borderRadius: 10, border: '1.5px solid var(--border)', background: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font)', color: 'var(--text-dark)' }}>View Profile</button>
          <button className="contact-btn" style={{ padding: '10px 14px', borderRadius: 10, border: 'none', background: 'var(--primary)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font)', color: 'white' }}>
            {isArrived ? 'Rate' : 'Contact'}
          </button>
        </div>
      </div>
      {/* Delivery agent for ongoing */}
      {!isArrived && (
        <div style={{ margin: '0 20px 16px', background: 'var(--bg)', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-subtle)', fontWeight: 500 }}>On the way to you</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
      ...
      {isArrived && (
        <div style={{ margin: '0 20px 16px', background: 'var(--bg)', borderRadius: 12, padding: '14px 16px' }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-subtle)', marginBottom: 6 }}>Order details</div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: 'var(--text-subtle)' }}>Payment</span>
            <span style={{ fontSize: 13, fontWeight: 600 }}>{order.payment}</span>
          </div>
        </div>
      )}
      <div style={{ padding: '8px 20px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {isArrived ? (
          <>
            <button className="btn btn-primary" style={{ fontSize: 15 }} onClick={() => {}}>Repeat Your Order</button>
            <button className="btn btn-outline" style={{ fontSize: 15 }} onClick={() => navigate(`/rate/${order.id}`)}>Rate Items</button>
          </>
        ) : (
          <>
            <button className="btn btn-primary" style={{ fontSize: 15 }} onClick={() => navigate('/order-tracking')}>Track Your Order</button>
            <button className="btn btn-outline" style={{ fontSize: 15 }} onClick={() => navigate('/home')}>Continue Shopping</button>
          </>
        )}
      </div>
    </div>
...
export function OrderTrackingPage() {
  ...
  return (
    <div className="screen" style={{ background: 'white' }}>
      ...
      {/* Real map: OpenStreetMap tile via staticmap service */}
      <div style={{ width: 'calc(100% - 40px)', height: 220, borderRadius: 12, margin: '16px 20px', overflow: 'hidden', position: 'relative', boxShadow: '0 2px 12px rgba(0,0,0,0.12)' }}>
        <img
          src="https://maps.geoapify.com/v1/staticmap?style=osm-bright&width=720&height=440&center=lonlat:77.0266,28.4595&zoom=14&marker=lonlat:77.0266,28.4595;color:%234CAF50;size:large&apiKey=a01e7ae86d5c4d72b68a0d4b4c45b2e6"
          alt="Delivery map"
          style={{ width: '100%', height: '100%', objectFit: 'cover' }}
          onError={e => {
            e.target.style.display = 'none'
            e.target.parentNode.style.background = 'linear-gradient(135deg,#c8e6c9,#81c784)'
            e.target.parentNode.innerHTML += '<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center"><div style="font-size:40px">📍</div><div style="font-size:13px;font-weight:600;color:#2e7d32;margin-top:8px">Gurgaon, Haryana</div><div style="font-size:12px;color:#388e3c;margin-top:4px">Arriving in 15 mins</div></div>'
          }}
        />
        <div style={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', background: 'white', borderRadius: 20, padding: '7px 18px', fontSize: 13, fontWeight: 600, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', whiteSpace: 'nowrap' }}>
          📍 Arriving in 15 mins
        </div>
      </div>
...
export function ConsumerSettingsPage() {
  const navigate = useNavigate()
  const [notifs, setNotifs] = useState({ orders: true, promos: false, newArrivals: true })
  ...
  return (
    <div className="screen" style={{ background: 'var(--bg)' }}>
        ...
        <div className="profile-menu-item" style={{ justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div className="profile-menu-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="var(--primary)" strokeWidth="1.8"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" stroke="var(--primary)" strokeWidth="1.8"/></svg>
            </div>
            <span style={{ fontWeight: 500 }}>Language</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-subtle)', fontSize: 14 }}>
            English <CaretRight size={16} color="var(--text-subtle)" />
          </div>
        </div>
        <div className="profile-menu-item" style={{ justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div className="profile-menu-icon">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" stroke="var(--primary)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
      ...
      <div style={{ background: 'white', marginBottom: 8 }}>
        ...
        {[
          { key: 'orders',     label: 'Order updates',    desc: 'Status updates on your orders' },
          { key: 'promos',     label: 'Promotions',       desc: 'Discounts, coupons and offers' },
          { key: 'newArrivals',label: 'New Arrivals',     desc: 'Fresh produce added by nearby farmers' },
        ].map(n => (
      ...
      <div style={{ background: 'white' }}>
        ...
        <div className="profile-menu-item" style={{ gap: 14 }} onClick={() => navigate('/splash')}>
          <div className="profile-menu-icon" style={{ background: '#ffebee' }}>
            <SignOut size={18} color="var(--error)" />
          </div>
